...
class TimeSeriesLoader(object):
    ...
    def _create_windows_tensor(self):
        tensor, right_padding, train_mask = self.ts_dataset.get_filtered_ts_tensor(
            offset=self.offset, output_size=self.output_size, window_sampling_limit=self.window_sampling_limit)
    ...
    def __iter__(self):
        ...
        n_batches = int(np.ceil(len(sample_idxs) / self.batch_size))  # Must be multiple of batch_size for parallel GPU
    ...
    def __get_item__(self, index):
        if self.model == 'nbeats':
            ...
            assert False, 'ESRNN model not implemented yet'
        else:
