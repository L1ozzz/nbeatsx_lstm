...
class TimeSeriesDataset(Dataset):
    def __init__(self,
        ...
        S_df: DataFrame
            DataFrame with static exogenous variables. Must contain columns ['unique_id']
        f_cols: list
            Name of columns which future exogenous variables (e.g., forecasts)
        ts_train_mask: list
    ...
    def _df_to_lists(self, Y_df, S_df, X_df):
        """
        Converts dataframes to lists for easier processing.
        """
        ...
        for i, u_id in enumerate(unique_ids):
            top_row = Y_df[Y_df['unique_id'] == u_id].index[0]
            bottom_row = Y_df[Y_df['unique_id'] == u_id].index[-1] + 1
            serie = Y_df.loc[top_row:bottom_row, 'y'].values
            last_ds_i = Y_df.loc[top_row:bottom_row, 'ds'].max()
            ...
            # X values
            for X_col in X_cols:
                serie = X_df.loc[top_row:bottom_row, X_col].values
            ...
            s_data_i = {}
            for S_col in S_cols:
                s_data_i[S_col] = S_df.loc[S_df['unique_id'] == u_id, S_col].values[0]
    ...
    def _create_tensor(self, ts_data, s_data):
        """
        Creates tensors for time series data.
        """
        ...
        for idx in range(self.n_series):
            ...
            print("ts_idx",ts_idx.shape)
            print("n_chanells",self.n_channels)
            print("idx", idx)
    ...
    def get_meta_data_col(self, col):
        """
        Retrieves metadata column values.
        """
    ...
    def get_filtered_ts_tensor(self, offset, output_size, window_sampling_limit, ts_idxs=None):
        """
        Filters tensor for time series data.
        """
        ...
        right_padding = max(last_outsample_ds - self.max_len, 0)  # To pad with zeros if there is "nothing" to the right
        ts_train_mask = self.ts_train_mask[first_ds:last_outsample_ds]

        assert np.sum(np.isnan(filtered_ts_tensor)) < 1.0, \
            f'The balanced filtered_tensor has {np.sum(np.isnan(filtered_ts_tensor))} nan values'
