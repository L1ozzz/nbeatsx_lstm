...
class TCN(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.padding == 'same':
            # keras-tcn 3.5.0 expects TensorShape, while Keras 3 may expose a
            # plain tuple here. Normalising to list preserves the same shape.
            output_shape_list = (self.build_output_shape.as_list()
                                 if hasattr(self.build_output_shape, 'as_list')
                                 else list(self.build_output_shape))
            time = output_shape_list[1]
            if time is not None:  # if time dimension is defined. e.g. shape = (bs, 500, input_dim).
                self.output_slice_index = int(output_shape_list[1] / 2)
            else:
        ...
        slicer_shape = (self.build_output_shape.as_list()
                        if hasattr(self.build_output_shape, 'as_list')
                        else list(self.build_output_shape))
        self.slicer_layer.build(slicer_shape)
